 ...
 * https://github.com/myMartek/JsonHelper
 */
public class JsonHelper implements Iterable<JsonHelper> {
    ...
    public JsonHelper(Reader content) {
        try {
            element = new JsonParser().parse(content);
        } catch (Exception e) {
            element = null;
    ...
    public <T> T convertToJavaObject(JsonHelper jsonObject, Class<T> classOfT) {
        Gson gson = new Gson();
        T object = gson.fromJson(jsonObject.jsonObject(), classOfT);
        return object;
    }
}